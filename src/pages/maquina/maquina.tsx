...
export default function PaginaFabrica(){
  ...
  const [maquina, setMaquina] = useState<{
    n_id_maquina: number, s_nome_maquina: string, s_codigo_maquina: string, n_ativo_maquina: string, s_nome_unidade: string, s_desc_statusmaquina:string,
    s_nome_fabrica:string
  }[]>([]);
    ...
    <div className='h-screen overflow-hidden'>
      ...
      <div className='h-[calc(100%-70px)] flex flex-col items-end'>
        <div className='bg-[#1394DA] md:w-[calc(100%-60px)] w-full h-[80px] flex justify-center items-end'>
          <p className='ml-[10%] mt-4 mb-4 text-2xl text-white'>Máquinas</p>
        ...
        <div className='w-[100%] md:w-[calc(100%-60px)] h-[calc(100%-100px)] flex flex-col items-center pb-5 md:overflow-hidden overflow-auto'>
          <p className='mt-5 md:ml-12 ml-8 md:mr-2 mr-5 mb-2 h-[20px]'>Lista de Máquinas</p>
          <div className='border-2 border-b-0 md:ml-10 md:mr-2 rounded-lg md:w-[70%] w-[95%] h-[calc(100%-20px)]'>
            <div className='bg-[#EEEEEE] h-[35px] rounded-t-lg flex md:pb-0 pb-7 pt-7 md:pt-0'>
              ...
              <p className='w-[50%] flex items-center justify-center text-sm text-center'>Unidade</p>
              <p className='w-[50%] flex items-center justify-center text-sm text-center'>Status</p>
            ...
            <div className='h-auto md:h-[calc(100%-35px)] overflow-auto'>
              {maquina.map((a,index)=>(
                <div className='border-b-2 h-[35px] flex' key={index}>
                  {/* <p className='w-[50%] flex items-center justify-center text-sm text-center'>{a.n_id_maquina}</p> */}
                  <p className='w-[50%] flex items-center justify-center text-sm text-center leading-none'>{a.s_nome_maquina}</p>
                  <p className='w-[50%] flex items-center justify-center text-sm text-center leading-none'>{a.s_codigo_maquina}</p>
                  <p className='w-[50%] flex items-center justify-center text-sm text-center leading-none'>{Number(a.n_ativo_maquina) == 1 ? "Sim" : "Não"}</p>
                  <p className='w-[50%] flex items-center justify-center text-sm text-center leading-none'>{a.s_nome_unidade}</p>
                  <p className='w-[50%] flex items-center justify-center text-sm text-center leading-none'>{a.s_desc_statusmaquina}</p>
